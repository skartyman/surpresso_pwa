          ...
          {!current ? <p>Выберите сотрудника из списка.</p> : (
            <>
              ...
              <section className="employees-form-section">
                ...
                <div className="employees-grid">
                  <label><span>Ёмкость нагрузки</span><input type="number" value={current.capacity ?? 6} onChange={(e) => (createMode ? setForm((p) => ({ ...p, capacity: Number(e.target.value) })) : setSelected((p) => ({ ...p, capacity: Number(e.target.value) })))} /></label>
                  <label><span>Макс. критических</span><input type="number" value={current.maxCritical ?? 2} onChange={(e) => (createMode ? setForm((p) => ({ ...p, maxCritical: Number(e.target.value) })) : setSelected((p) => ({ ...p, maxCritical: Number(e.target.value) })))} /></label>
                  <label><span>Вес приоритета</span><input type="number" value={current.priorityWeight ?? 0} onChange={(e) => (createMode ? setForm((p) => ({ ...p, priorityWeight: Number(e.target.value) })) : setSelected((p) => ({ ...p, priorityWeight: Number(e.target.value) })))} /></label>
                  <label><span>Берёт срочные</span><select value={current.canTakeUrgent ? 'true' : 'false'} onChange={(e) => (createMode ? setForm((p) => ({ ...p, canTakeUrgent: e.target.value === 'true' })) : setSelected((p) => ({ ...p, canTakeUrgent: e.target.value === 'true' })))}><option value="true">Да</option><option value="false">Нет</option></select></label>
                  <label><span>Берёт выездные заявки</span><select value={current.canTakeFieldRequests ? 'true' : 'false'} onChange={(e) => (createMode ? setForm((p) => ({ ...p, canTakeFieldRequests: e.target.value === 'true' })) : setSelected((p) => ({ ...p, canTakeFieldRequests: e.target.value === 'true' })))}><option value="true">Да</option><option value="false">Нет</option></select></label>
                </div>
              </section>
